#!/bin/bash
# Copyright (c) 2013 The Sikacoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Sikacoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Sikacoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Sikacoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Sikacoin 0.7 datadir."
    ;;
  3)
    echo "Detected Sikacoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Sikacoin 0.8 datadir."
    ;;
esac
