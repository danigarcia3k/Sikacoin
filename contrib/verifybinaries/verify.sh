...
###   sikacoincore.org and sikacoin.org and compares them.
...
WORKINGDIR="/tmp/sikacoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://sikacoincore.org"
HOST2="https://sikacoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="sikacoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'sikacoin-',
...
if [ $? -ne 0 ]; then
   echo "sikacoin.org failed to provide signature file, but sikacoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "sikacoin.org and sikacoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Sikacoin Core binary release signing key installed?"
   fi
