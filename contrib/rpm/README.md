...
The RPM spec file provided here is for Sikacoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://sikacoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Sikacoin git repository](https://github.com/sikacoin/sikacoin) but are not part of
...
Currently only `Source10` is used, for the example `sikacoin.conf` file.
...
in the sikacoin git repository. At present only an SVG version of the Sikacoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the sikacoin
...
needed when 0.12.1 is released, a proper fix is already in the Sikacoin git
master branch.
...
idea to build Sikacoin Core against a static version of that library compiled
...
The `sikacoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `sikacoin-server` package. At this point, most RPM based
...
The `sikacoin-server` package should have SELinux support. How to properly do
...
Sikacoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/sikacoin/). He has been
packaging Sikacoin for Fedora at least since 2012.
...
4. I renamed the `sikacoin` package that contains the Qt GUI to `sikacoin-core` as
that appears to be how the general population refers to it, in contrast to
`sikacoin-xt` or `sikacoin-classic`. I wanted to make sure the general population
