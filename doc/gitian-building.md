...
*Setup instructions for a Gitian build of Sikacoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Sikacoin
...
- [Building Sikacoin Core](#building-sikacoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-sikacoin-host/README.md).
...
Clone the git repositories for sikacoin and Gitian.
...
in the sikacoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Sikacoin Core
----------------

To build Sikacoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the sikacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/sikacoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/sikacoin/sikacoin
...
For example:
```bash
URL=https://github.com/laanwj/sikacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit sikacoin=${COMMIT} --url sikacoin=${URL} ../sikacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit sikacoin=${COMMIT} --url sikacoin=${URL} ../sikacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit sikacoin=${COMMIT} --url sikacoin=${URL} ../sikacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the sikacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../sikacoin/contrib/gitian-descriptors/*|sort|uniq )
...
SKCPATH=/some/root/path/sikacoin
SIGPATH=/some/root/path/sikacoin-detached-sigs

./bin/gbuild --url sikacoin=${SKCPATH},signature=${SIGPATH} ../sikacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/sikacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/sikacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/sikacoin-osx-build.assert
```
...
[sikacoin-core/gitian.sigs](https://github.com/sikacoin-core/gitian.sigs/) repository, or if that's not possible create a pull
